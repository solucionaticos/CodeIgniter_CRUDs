...
class Languages_variables extends MY_Controller {
	...
	public function index() {
		...
		$this->parameters['path'] = 'backend/languages_variables';
		$this->parameters['title'] = 'Languages Variables';
		$this->parameters['subtitle'] = '';
		$this->parameters['head_title'] = $this->parameters['title'];
		$this->parameters['breadcrumb'] = '
		<ol class="breadcrumb">
			<li><a href="#"><i class="fa fa-dashboard"></i> Inicio</a></li>
			<li class="active">Languages Variables</li>
		</ol>'; 
	...
	public function getRecord () {
		...
		$registro = $this->Model->registro('languages_variables', $post['id']);
	...
	public function insert () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$id = $this->Model->insertar('languages_variables', $datos);
				if ($id > 0) {
					...
					redirect(base_url() . 'backend/languages_variables');
				} else {
		...
		redirect(base_url() . 'backend/languages_variables/new');
	}
	...
	public function update () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($this->Model->actualizar('languages_variables', $datos, $post['id'])) {
		...
		redirect(base_url() . 'backend/languages_variables/edit/'.$post['id']);   
	}
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				foreach ($mat_id as $id) {
					$this->Model->eliminar('languages_variables', $id);
					$proceso = true;
				}
	...
	public function list () {
		...
		$registros = $this->Model->registros('languages_variables');
