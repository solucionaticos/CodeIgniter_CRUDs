...
class Datos_dispositivos extends MY_Controller {
	...
	public function index() {
		...
		$this->parameters['path'] = $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_dispositivos';
		$this->parameters['title'] = 'Datos Dispositivos';
		$this->parameters['subtitle'] = '';
		$this->parameters['head_title'] = $this->parameters['title'];
		$this->parameters['breadcrumb'] = '
		<ol class="breadcrumb">
			<li><a href="#"><i class="fa fa-dashboard"></i> Inicio</a></li>
			<li class="active">Datos Dispositivos</li>
		</ol>'; 
	...
	public function getRecord () {
		...
		$registro = $this->Model->registro('datos_dispositivos', $post['id']);
	...
	public function insert () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$id = $this->Model->insertar('datos_dispositivos', $datos);
				if ($id > 0) {
					...
					redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_dispositivos');
				} else {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_dispositivos/new');
	}
	...
	public function update () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($this->Model->actualizar('datos_dispositivos', $datos, $post['id'])) {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_dispositivos/edit/'.$post['id']);   
	}
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				foreach ($mat_id as $id) {
					$this->Model->eliminar('datos_dispositivos', $id);
					$proceso = true;
				}
	...
	public function list () {
		...
		$registros = $this->Model->registros('datos_dispositivos');
