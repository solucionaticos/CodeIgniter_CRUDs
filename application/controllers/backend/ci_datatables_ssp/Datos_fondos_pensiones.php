...
class Datos_fondos_pensiones extends MY_Controller {
	...
	public function index() {
		...
		$this->parameters['path'] = $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_fondos_pensiones';
		$this->parameters['title'] = 'Datos Fondos Pensiones';
		$this->parameters['subtitle'] = '';
		$this->parameters['head_title'] = $this->parameters['title'];
		$this->parameters['breadcrumb'] = '
		<ol class="breadcrumb">
			<li><a href="#"><i class="fa fa-dashboard"></i> Inicio</a></li>
			<li class="active">Datos Fondos Pensiones</li>
		</ol>'; 
	...
	public function getRecord () {
		...
		$registro = $this->Model->registro('datos_fondos_pensiones', $post['id']);
	...
	public function insert () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$datos = array(
						"pais_is" => $post["pais_is"],
						"nombre" => $post["nombre"],);

				$id = $this->Model->insertar('datos_fondos_pensiones', $datos);
				if ($id > 0) {
					...
					redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_fondos_pensiones');
				} else {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_fondos_pensiones/new');
	}
	...
	public function update () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($this->Model->actualizar('datos_fondos_pensiones', $datos, $post['id'])) {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_fondos_pensiones/edit/'.$post['id']);   
	}
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				foreach ($mat_id as $id) {
					$this->Model->eliminar('datos_fondos_pensiones', $id);
					$proceso = true;
				}
	...
	public function list () {
		...
		$registros = $this->Model->registros('datos_fondos_pensiones');
