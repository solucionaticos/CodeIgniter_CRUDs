...
class Datos_afc_cuentas extends MY_Controller {
	...
	public function index() {
		...
		$this->parameters['path'] = $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_afc_cuentas';
		$this->parameters['title'] = 'Datos Afc Cuentas';
		$this->parameters['subtitle'] = '';
		$this->parameters['head_title'] = $this->parameters['title'];
		$this->parameters['breadcrumb'] = '
		<ol class="breadcrumb">
			<li><a href="#"><i class="fa fa-dashboard"></i> Inicio</a></li>
			<li class="active">Datos Afc Cuentas</li>
		</ol>'; 
	...
	public function getRecord () {
		...
		$registro = $this->Model->registro('datos_afc_cuentas', $post['id']);
	...
	public function insert () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$id = $this->Model->insertar('datos_afc_cuentas', $datos);
				if ($id > 0) {
					...
					redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_afc_cuentas');
				} else {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_afc_cuentas/new');
	}
	...
	public function update () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($this->Model->actualizar('datos_afc_cuentas', $datos, $post['id'])) {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_afc_cuentas/edit/'.$post['id']);   
	}
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				foreach ($mat_id as $id) {
					$this->Model->eliminar('datos_afc_cuentas', $id);
					$proceso = true;
				}
	...
	public function list () {
		...
		$registros = $this->Model->registros('datos_afc_cuentas');
