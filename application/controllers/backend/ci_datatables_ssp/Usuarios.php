...
class Usuarios extends MY_Controller {
	...
	public function index() {
		...
		$this->parameters['path'] = $this->config->item('adminPath') . '/backend/ci_datatables_ssp/usuarios';
		$this->parameters['title'] = 'Usuarios';
	...
	public function new() {
		...
		$tabla_datos_usuarios_perfiles = $this->Model->registros("datos_usuarios_perfiles", "id,nombre nombre", array(), "nombre" );
		$this->parameters["datos"]["tabla_datos_usuarios_perfiles"] = $tabla_datos_usuarios_perfiles;
	...
	public function getRecord () {
		...
		$registro = $this->Model->registro('usuarios', $post['id']);
	...
	public function insert () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$datos = array(
						...
						"fecha_ultimo_ingreso" => $post["fecha_ultimo_ingreso"],
						"ip" => $post["ip"],
						"fecha_creacion" => $post["fecha_creacion"],
						"activo" => $post["activo"],);

				$id = $this->Model->insertar('usuarios', $datos);
				if ($id > 0) {
					...
					redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/usuarios');
				} else {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/usuarios/new');
	}
	...
	public function update () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($this->Model->actualizar('usuarios', $datos, $post['id'])) {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/usuarios/edit/'.$post['id']);   
	}
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				foreach ($mat_id as $id) {
					$this->Model->eliminar('usuarios', $id);
					$proceso = true;
				}
	...
	public function list () {
		...
		$registros = $this->Model->registros('usuarios');
