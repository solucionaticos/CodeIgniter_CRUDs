...
class Datos_frecuencias_pagos extends MY_Controller {
	...
	public function index() {
		...
		$this->parameters['path'] = $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_frecuencias_pagos';
		$this->parameters['title'] = 'Datos Frecuencias Pagos';
		$this->parameters['subtitle'] = '';
		$this->parameters['head_title'] = $this->parameters['title'];
		$this->parameters['breadcrumb'] = '
		<ol class="breadcrumb">
			<li><a href="#"><i class="fa fa-dashboard"></i> Inicio</a></li>
			<li class="active">Datos Frecuencias Pagos</li>
		</ol>'; 
	...
	public function getRecord () {
		...
		$registro = $this->Model->registro('datos_frecuencias_pagos', $post['id']);
	...
	public function insert () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$id = $this->Model->insertar('datos_frecuencias_pagos', $datos);
				if ($id > 0) {
					...
					redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_frecuencias_pagos');
				} else {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_frecuencias_pagos/new');
	}
	...
	public function update () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($this->Model->actualizar('datos_frecuencias_pagos', $datos, $post['id'])) {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/datos_frecuencias_pagos/edit/'.$post['id']);   
	}
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				foreach ($mat_id as $id) {
					$this->Model->eliminar('datos_frecuencias_pagos', $id);
					$proceso = true;
				}
	...
	public function list () {
		...
		$registros = $this->Model->registros('datos_frecuencias_pagos');
