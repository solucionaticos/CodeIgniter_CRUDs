...
class Empresa_centros_costo extends MY_Controller {
	...
	public function index() {
		...
		$this->parameters['path'] = $this->config->item('adminPath') . '/backend/ci_datatables_ssp/empresa_centros_costo';
		$this->parameters['title'] = 'Empresa Centros Costo';
		$this->parameters['subtitle'] = '';
		$this->parameters['head_title'] = $this->parameters['title'];
		$this->parameters['breadcrumb'] = '
		<ol class="breadcrumb">
			<li><a href="#"><i class="fa fa-dashboard"></i> Inicio</a></li>
			<li class="active">Empresa Centros Costo</li>
		</ol>'; 
	...
	public function getRecord () {
		...
		$registro = $this->Model->registro('empresa_centros_costo', $post['id']);
	...
	public function insert () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$id = $this->Model->insertar('empresa_centros_costo', $datos);
				if ($id > 0) {
					...
					redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/empresa_centros_costo');
				} else {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/empresa_centros_costo/new');
	}
	...
	public function update () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($this->Model->actualizar('empresa_centros_costo', $datos, $post['id'])) {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/backend/ci_datatables_ssp/empresa_centros_costo/edit/'.$post['id']);   
	}
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				foreach ($mat_id as $id) {
					$this->Model->eliminar('empresa_centros_costo', $id);
					$proceso = true;
				}
	...
	public function list () {
		...
		$registros = $this->Model->registros('empresa_centros_costo');
