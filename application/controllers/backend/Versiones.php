...
class Versiones extends MY_Controller {
	...
	public function index() {
		...
		$this->parameters['path'] = 'backend/versiones';
	...
	public function getRecord () {
		...
		$registro = $this->Model->registro('versiones', $post['id']);
	...
	public function insert () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$id = $this->Model->insertar('versiones', $datos);
				if ($id > 0) {
					...
					redirect(base_url() . 'backend/versiones');
				} else {
		...
		redirect(base_url() . 'backend/versiones/new');
	}
	...
	public function update () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($this->Model->actualizar('versiones', $datos, $post['id'])) {
		...
		redirect(base_url() . 'backend/versiones/edit/'.$post['id']);   
	}
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				foreach ($mat_id as $id) {
					$this->Model->eliminar('versiones', $id);
					$proceso = true;
				}
	...
	public function list () {
		...
		$registros = $this->Model->registros('versiones');
	...
	public function list_ssp () {
		...
		$columns = array(array( 'db' => 'id', 'dt' => 0, 'field' => 'id', 'formatter' => function($d, $row) {return '<input type="checkbox" id="fila_' . $d . '" class="seleccion" cod="' . $d . '">';}),array( 'db' => 'id', 'dt' => 1, 'field' => 'id', 'formatter' => function($d, $row) {return '<button type="button" class="btn btn-default btn-xs text-light-blue btnEditar" cod="' . $d . '"><span class="glyphicon glyphicon-pencil"></span></button>';}), array( 'db' => 'id', 'dt' => 2, 'field' => 'id', 'formatter' => function($d, $row) {return '<button type="button" class="btn btn-default btn-xs text-red btnEliminar" cod="' . $d . '"><span class="glyphicon glyphicon-trash"></span></button>';}), array( 'db' => 'proyecto', 'dt' => 3, 'field' => 'proyecto',
				"formatter" => function($d, $row) {
							 ...
							 ),array( 'db' => 'nombre', 'dt' => 4, 'field' => 'nombre' ),);
