...
class Datos extends MY_Controller {
	...
	public function index() {
		...
		$this->parameters['path'] = 'backend/datos';
		$this->parameters['title'] = 'Datos';
	...
	public function getRecord () {
		...
		$registro = $this->Model->registro('datos', $post['id']);
	...
	public function insert () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$datos = array(
						"nombre" => $post["nombre"],
						"codigo" => $post["codigo"],
						"numero_registros" => $post["numero_registros"],
						"created_at" => $post["created_at"],
						"updated_at" => $post["updated_at"],);

				$id = $this->Model->insertar('datos', $datos);
				if ($id > 0) {
					...
					redirect(base_url() . 'backend/datos');
				} else {
		...
		redirect(base_url() . 'backend/datos/new');
	}
	...
	public function update () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($this->Model->actualizar('datos', $datos, $post['id'])) {
		...
		redirect(base_url() . 'backend/datos/edit/'.$post['id']);   
	}
	...
	public function list () {
		...
		$registros = $this->Model->registros('datos');
