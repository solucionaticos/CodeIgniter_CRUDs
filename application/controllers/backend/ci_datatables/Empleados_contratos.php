...
class Empleados_contratos extends MY_Controller {
	...
	public function index() {
		...
		$this->parameters['vista'] = 'backend/ci_datatables/empleados_contratos';
		$this->parameters['datos']['titulo'] = 'Empleados Contratos';
		...
		$this->admin_design->layout('database', 'backend/ci_datatables/empleados_contratos', $this->parameters, 'Empleados Contratos');
	}

	public function traerRegistro () {
		$post = $this->input->post(NULL, TRUE);
		$registro = $this->Model->registro('empleados_contratos', $post['id']);
	...
	public function ingresar () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$id = $this->Model->insertar('empleados_contratos', $datos);
				if ($id > 0) {
		...
		redirect('/backend/ci_datatables/empleados_contratos');
	}
	...
	public function guardar () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($this->Model->actualizar('empleados_contratos', $datos, $post['id'])) {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				foreach ($mat_id as $id) {
					$this->Model->eliminar('empleados_contratos', $id);
					$proceso = true;
				}
	...
	public function lista () {
		...
		$registros = $this->Model->registros('empleados_contratos');
	...
	public function lista_ssp () {
		...
		$joinQuery = "FROM empleados_contratos";
