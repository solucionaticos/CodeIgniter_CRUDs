...
class Apis_nodejs_express extends MY_Controller {
...
'paging': false,
	...
	$('#generateSelectRows').click(function () {
		...
		ids += '0';
		$.ajax({
			url: proyVar.base_url + proyVar.admin_path + '/cruds/apis_nodejs_express/generate_list_ids',	
			cache: false,
			type: 'post',
			data: {'ids':ids},
			success:function(datos) {
				//tksec = datos.tksec;
				location.href = proyVar.base_url + proyVar.admin_path + '/cruds/apis_nodejs_express';
			}
		});
...
";
		...
		// Views
		$this->admin_design->_load_layout($this->config->item('adminPath') . '/cruds/apis_nodejs_express/cruds', $data);
...
// -------------------------------------------------------------------------------------
        ...
        $this->admin_design->_load_layout($this->config->item('adminPath') . '/cruds/apis_nodejs_express/crud', $data);
...
// -------------------------------------------------------------------------------------
        ...
        $this->admin_design->_load_layout($this->config->item('adminPath') . '/cruds/apis_nodejs_express/crud_cruds', $data);
    ...
    public function generate($id) {
		...
		redirect($this->config->item('adminPath') . '/cruds/apis_nodejs_express/crud/' . $id);
    ...
    public function generate_cruds($id) {
		...
		$this->_generateCode($id);
		redirect($this->config->item('adminPath') . '/cruds/apis_nodejs_express/crud_cruds/' . $id);
	...
	public function generate_list($id) {
		...
		$this->_generateCode($id);
		redirect($this->config->item('adminPath') . '/cruds/apis_nodejs_express');
    ...
    function _generateCode($id) {
		...
		$path = 'assets/cruds/apis_nodejs_express/' . $project_name;
		if (!is_dir($path)) {
		...
		// ---------------------------------------------------------------------------------------------------------		
		// $path = 'assets/cruds/apis_nodejs_express/' . $project_name . '/' . $version_name . '/server/ssh';
		...
		// $code_routes
		// ---------------------------------------------------------------------------------------------------------		
		// $path = 'assets/cruds/apis_nodejs_express/' . $project_name . '/' . $version_name . '/server';
		...
		// $code_models
		// ---------------------------------------------------------------------------------------------------------		
		$path = 'assets/cruds/apis_nodejs_express/' . $project_name . '/' . $version_name;
		...
		// $code_controllers
		// ---------------------------------------------------------------------------------------------------------		
		$path = 'assets/cruds/apis_nodejs_express/' . $project_name . '/' . $version_name . '/bin/src/app';
		...
		// ---------------------------------------------------------------------------------------------------------		
		// $path = 'assets/cruds/apis_nodejs_express/' . $project_name . '/' . $version_name . '/server/app';
		...
		// ---------------------------------------------------------------------------------------------------------		
		// $path = 'assets/cruds/apis_nodejs_express/' . $project_name . '/' . $version_name . '/server/app/Validators';
...
// ';
						...
						$code_controllers_fields .= '        const ' . $regCampo['nombre'] . ' = httpReq.body.' . $regCampo['nombre'] . ';
';
						$code_controllers_fields_obj .= '            ' . $regCampo['nombre'] . ': ' . $regCampo['nombre'] . ',
';
...
// }).prefix("api/v1");';
			...
			$code_routes = "var express = require('express');
var router = express.Router();
var TableController = require('../bin/src/app/Http/Controllers/".$tableNameCamelCase."Controller');

router.get('/', TableController.index );
router.get('/:id', TableController.show );
router.get('/:field/:value', TableController.findBy);
router.get('/:field/:operator/:value', TableController.queryWhere);
router.get('/:field/:operator/:value/:count', TableController.queryWheregetCount);
router.post('/', TableController.store);
router.put('/:id', TableController.update);
router.delete('/:id', TableController.destroy);
router.delete('/:field/:operator/:value', TableController.deletes);
router.delete('/', TableController.truncate);
...
// ';
			...
			$code_models = "const Model = require('./Model');

class ".$tableNameCamelCase."Model extends Model{
    constructor(){
        super();
...
module.exports = new ".$tableNameCamelCase."Model();";
...
const EMessages = require('../../Constants/EMessages');
const TableModel = require('../../Models/".$tableNameCamelCase."Model');
...
".$code_controllers_fields_obj."
        ...
        .then(row => {
            console.log(row);
            httpRes.json(new Response(EMessages.INSERT).setData(row));
        });
    }    
...
".$code_controllers_fields_obj."
        ...
        .where(TableModel.primaryKey, id)
        .then(row => {
            console.log(row);
            httpRes.json(new Response(EMessages.UPDATE).setData(row));
        });
    }

}

const control = new TableController();
