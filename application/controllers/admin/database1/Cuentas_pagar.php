...
class Cuentas_pagar extends MY_Controller {
	...
	public function index() {
		...
		$this->parameters['vista'] = $this->config->item('adminPath') . '/database1/cuentas_pagar';
		$this->parameters['datos']['titulo'] = 'Cuentas Pagar';
		$this->parameters['datos']['subtitulo'] = ''; 

		$this->admin_design->layout('database',  $this->config->item('adminPath') . '/database1/cuentas_pagar', $this->parameters, 'Cuentas Pagar');
	}

	public function traerRegistro () {
		$post = $this->input->post(NULL, TRUE);
		$registro = $this->Model->registro('cuentas_pagar', $post['id']);
	...
	public function ingresar () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$id = $this->Model->insertar('cuentas_pagar', $datos);
				if ($id > 0) {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/database1/cuentas_pagar');
	}
	...
	public function guardar () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($this->Model->actualizar('cuentas_pagar', $datos, $post['id'])) {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				foreach ($mat_id as $id) {
					$this->Model->eliminar('cuentas_pagar', $id);
					$proceso = true;
				}
	...
	public function lista () {
		...
		$registros = $this->Model->registros('cuentas_pagar');
