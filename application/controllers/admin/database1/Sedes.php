...
class Sedes extends MY_Controller {
	...
	public function index() {
		...
		$this->parameters['vista'] = $this->config->item('adminPath') . '/database1/sedes';
		$this->parameters['datos']['titulo'] = 'Sedes';
		...
		$this->admin_design->layout('database', $this->config->item('adminPath') . '/database1/sedes', $this->parameters, 'Sedes');
	}

	public function traerRegistro () {
		$post = $this->input->post(NULL, TRUE);
		$registro = $this->Model->registro('sedes', $post['id']);
	...
	public function ingresar () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$id = $this->Model->insertar('sedes', $datos);
				if ($id > 0) {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/database1/sedes');
	}
	...
	public function guardar () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($this->Model->actualizar('sedes', $datos, $post['id'])) {
	...
	public function lista () {
		...
		$registros = $this->Model->registros('sedes');
