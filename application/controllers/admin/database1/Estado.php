...
class Estado extends MY_Controller {
	...
	public function index() {
		...
		$this->parameters['vista'] = $this->config->item('adminPath') . '/database1/estado';
		$this->parameters['datos']['titulo'] = 'Estado';
		$this->parameters['datos']['subtitulo'] = ''; 

		$tabla_pais = $this->Model->registros("pais", "id,nombre nombre", array(), "nombre" );
		$this->parameters["datos"]["tabla_pais"] = $tabla_pais;
		$this->admin_design->layout('database', $this->config->item('adminPath') . '/database1/estado', $this->parameters, 'Estado');
	}

	public function traerRegistro () {
		$post = $this->input->post(NULL, TRUE);
		$registro = $this->Model->registro('estado', $post['id']);
	...
	public function ingresar () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$id = $this->Model->insertar('estado', $datos);
				if ($id > 0) {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/database1/estado');
	}
	...
	public function guardar () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($this->Model->actualizar('estado', $datos, $post['id'])) {
	...
	public function lista () {
		...
		$registros = $this->Model->registros('estado');
