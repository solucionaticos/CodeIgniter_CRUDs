...
class Versiones extends MY_Controller {
	...
	public function index() {
		...
		$this->parametros['vista'] = $this->config->item('adminPath') . '/versiones';
		$this->parametros['datos']['titulo'] = 'versiones';
		$this->parametros['datos']['subtitulo'] = ''; 
		$tabla_versiones = $this->Modelo->registros('versiones', '', array() );
		$this->parametros['datos']['lista'] = $tabla_versiones;

		$tabla_proyectos = $this->Modelo->registros("proyectos", "id,nombre nombre", array(), "nombre" );
		$this->parametros["datos"]["tabla_proyectos"] = $tabla_proyectos;
	...
	public function traerRegistro () {
		...
		$registro = $this->Modelo->registro('versiones', $post['id']);
	...
	public function ingresar () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$id = $this->Modelo->insertar('versiones', $datos);
				if ($id > 0) {
		...
		redirect(base_url() . $this->config->item('adminPath') . '/versiones');
	}
	...
	public function guardar () {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($this->Modelo->actualizar('versiones', $datos, $post['id'])) {
		...
		if (!empty($post)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				foreach ($mat_id as $id) {
					$this->Modelo->eliminar('versiones', $id);
					$proceso = true;
				}
	...
	public function lista () {
		...
		$registros = $this->Modelo->registros('versiones');
